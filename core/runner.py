...
class Runner:
    ...
    def run(self) -> int:
        ...
        try:
            while not self.stop_event.is_set():
                ...
                # 等這一波排定時刻與補跑清完再接著跑。
                #
                # ⚠ 這一段不修的話，副本讓位一次就等於整天只打到一半：實測另一位
                #   使用者 08:30 開打，09:04 拿到第一次 S 領完獎、剛好撞上 09:00
                #   的排程而讓位，第二次領獎次數就一路留到隔天。而紀錄上每一行
                #   都正常。
    ...
    def _passed_during(
        ...
        ⚠ **起點要退到整分鐘。** `start` 是 `datetime.now()`，取得的時間點在
          引擎建好之後——實測是 08:00:00.213，而排定時刻是 08:00:00.000。
          直接比的話，**和這一輪排在同一分鐘的腳本永遠差 0.2 秒落在區間外**。
          實測 2026-08-24：副本與競技場都排 08:00，副本先跑到 09:16，
          期間只撈到 09:00 的雜務，競技場整格消失——而那天是週一，
          「開啟挑戰」與上一期的排名獎勵一起沒了。
